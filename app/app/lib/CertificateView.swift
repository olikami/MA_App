  ...
  var body: some View {
    VStack {
      if let firstCertificate = certificates.first {
        Text(firstCertificate.subject.description)
      }
    ...
    .overlay(
      OverlayView(certificates: certificates)
        .background(Color.white.opacity(0.95))
        .padding()
        .cornerRadius(15)
        .shadow(radius: 5)
        .offset(y: showOverlay ? 0 : UIScreen.main.bounds.height),
...
struct OverlayView: View {
  ...
  var body: some View {
    VStack(spacing: 20) {
      ForEach(certificates.reversed(), id: \.subject.description) { certificate in
